...
# 가격(price)만 0~1 사이로 정규화
# ------------------------------------------------------------
scaler = MinMaxScaler()
df['price_scaled'] = scaler.fit_transform(df[['price']])

# ------------------------------------------------------------
# 7단계: 파생변수 생성 - 예약가능성 그룹 생성
# 가능/매일 가능/불가능 구간별로 묶기
# ------------------------------------------------------------
def availability_365_group(availability_365):
    if availability_365 > 364:
        return 'everyday'
    elif availability_365 > 0:
        return 'posible'
    else:
        return 'imposible'
...
# ------------------------------------------------------------
# 8단계: 예약 가능성 가격 비교 시각화 (matplotlib 사용)
# ------------------------------------------------------------
availability_price_rate = df.groupby('availability_365_group')['price_scaled'].mean().sort_index()
